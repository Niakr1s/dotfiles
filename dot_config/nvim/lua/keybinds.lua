...
keymap("n", "<leader>qr", "<cmd>restart<cr>", { desc = 'Restart' })
keymap("n", "ö", ":")
keymap("i", "<C-S-v>", "<C-r><C-o>*", { desc = 'Paste from System in Insertmode' })
keymap("n", "<leader>mu", function() vim.pack.update() end, { desc = 'Update Plugins' })
...
{ desc = 'Find Help' })
keymap("n", "<leader>fH", function() require('mini.extra').pickers.hl_groups() end,
{ desc = 'Find HL Groups' })
...
keymap("n", "<leader>bo", "<cmd>so<cr>", { desc = 'Source' })
...
keymap("n", "<leader>um", "<cmd>lua MiniMap.toggle()<cr>", { desc = 'Mini Map' })
